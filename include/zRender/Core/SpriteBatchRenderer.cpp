...
namespace zRender
{
	...
	void SpriteBatchRenderer::DrawTextAr(
		UnicodeFont* font,
		...
		Color color)
	{
		...
		if (textureIndex == 0.0f)
		{
			if (s_Data.textureSlotIndex >= MaxTextures - 1 || s_Data.indexCount >= MaxIndexCount)
			{
				End();
				Flush();
				Begin();
		...
		float scaleFactor = size / 48.0f; // افترضنا أن 48 هو حجم الـ font الأساسي
		...
		for (auto cp : codepoints)
		{
			...
			// حجم الحرف بعد scale
			float w = g.size.x * scaleFactor;
			float h = g.size.y * scaleFactor;

			// موقع الحرف بالنسبة لـ top-left
			float x = pos.x + g.bearing.x * scaleFactor;
			float y = pos.y - (g.size.y - g.bearing.y) * scaleFactor;

			// إنشاء quad
			SpriteQuad quad{};
			quad.layer = 0;

			quad.vertices[0].position = { x,     y };
			quad.vertices[1].position = { x + w, y };
			quad.vertices[2].position = { x + w, y + h };
			quad.vertices[3].position = { x,     y + h };

			quad.vertices[0].uv = g.uvMin;
			quad.vertices[1].uv = { g.uvMax.x, g.uvMin.y };
			quad.vertices[2].uv = g.uvMax;
			quad.vertices[3].uv = { g.uvMin.x, g.uvMax.y };
			...
			// RTL: الحرف التالي يذهب لليسار
			pos.x -= g.advance * scaleFactor;
		}
